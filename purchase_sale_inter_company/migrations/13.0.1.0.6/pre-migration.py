...
def _pre_create_columns(env):
    ...
    if not openupgrade.column_exists(
        env.cr, "stock_picking", "is_intercompany_picking"
    ):
        openupgrade.logged_query(
            env.cr,
            """
            ALTER TABLE stock_picking
            ADD COLUMN is_intercompany_picking BOOLEAN;
            COMMENT ON COLUMN stock_picking.is_intercompany_picking
            IS 'Is Inter Company Transfer?';
            """,
        )
