...
class AccountPaymentOrder(models.Model):
    ...
    def _create_move_line_suspense_account(
        self, payment_line, bank_journal, move, dest_company
    ):
        vals = {
            "move_id": move.id,
    ...
    def _create_move_lines(self, payment_line, bank_journal, move, dest_company):
        move_lines = self._create_move_line_suspense_account(
            payment_line, bank_journal, move, dest_company
        )
    ...
    def _prepare_move_vals(self, dest_company, bank_journal, payment_line):
        vals = {
    ...
    def _create_move(self, dest_company, bank_journal, payment_line):
        vals = self._prepare_move_vals(dest_company, bank_journal, payment_line)
    ...
    def generated2uploaded(self):
        ...
        for payment_line in self.payment_line_ids:
            dest_company = (
                ...
                .sudo()
                .search([("partner_id", "=", payment_line.partner_id.id)], limit=1)
            )
            if dest_company:
                bank_journal = self.env["account.journal"].search(
                    [
                        ...
                        (
                            ...
                            "=",
                            payment_line.partner_bank_id.sanitized_acc_number,
                        ),
                    ],
                    limit=1,
                )
                move = self._create_move(dest_company, bank_journal, payment_line)
