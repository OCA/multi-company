                ...
                for distrib in invoice_line.with_user(SUPERUSER_ID).distribution_ids:
                    ...
                    if distrib.company_id != self.company_id:
                        if inv.move_type == "in_invoice":
                            ...
                            # company
                            self.get_from_lines(
                                from_lines, invoice_line, inv, amount, distrib, True
                            )
    ...
    def action_reverse(self):
        ...
        for line in self.invoice_line_ids:
            for dist_line in line.distribution_ids:
                if (
                    dist_line.company_id
                    != self.invoice_line_ids[index].distribution_ids.company_id
                ):
                    dist_obj.create(
                        {
            ...
            from_lines.append(
                (
                    ...
                    0,
                    {
                        ...
                        "debit": amount,
                        "account_id": invoice_id.company_id.due_from_account_id.id,
                        "partner_id": distribution.company_id.partner_id.id,
                    },
                )
            )
            ...
            from_lines.append(
                (
                    ...
                    0,
                    {
                        ...
                        "partner_id": invoice_id.company_id.partner_id.id,
                    },
                )
            )
        else:
